...
import { isEmptyLoci, Loci } from '../../mol-model/loci.js';
import { Wiggle } from '../../mol-theme/wiggle.js';
import { OrderedSet } from '../../mol-data/int.js';
const WiggleManagerTag = 'wiggle-controls';
export async function setStructureWiggle(plugin, components, value, lociGetter, types) {
...
export async function clearStructureWiggle(plugin, components, types) {
    await eachRepr(plugin, components, async (update, repr, wiggleCell) => {
        if (types && types.length > 0 && !types.includes(repr.params.values.type.name))
            return;
        if (wiggleCell) {
...
function getFilteredBundle(layers, structure) {
    ...
    return Wiggle.filter(merged, structure);
}
 ...
 */
export async function setStructureWiggleFromUncertainty(plugin, components, scale = 1, types) {
    await eachRepr(plugin, components, async (update, repr, wiggleCell) => {
        if (types && types.length > 0 && !types.includes(repr.params.values.type.name))
            return;
        const structure = repr.obj.data.sourceData;
        ...
        for (const [bucket, unitIndices] of buckets) {
            ...
            for (const { unit, indices } of unitIndices) {
                elements.push({
                    unit,
                    indices: OrderedSet.ofSortedArray(new Int32Array(indices)),
                });
            }
