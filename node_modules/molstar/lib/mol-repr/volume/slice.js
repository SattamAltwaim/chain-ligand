...
import { transformPositionArray } from '../../mol-geo/util.js';
import { Color } from '../../mol-util/color/index.js';
import { ColorTheme } from '../../mol-theme/color.js';
import { packIntToRGBArray } from '../../mol-util/number-packing.js';
import { createPeriodicVolume, createVolumeSliceCellLocationIterator, eachVolumeLoci } from './util.js';
...
import { cantorPairing } from '../../mol-data/util/hash-functions.js';
...
export function applySliceObjectLoci(loci, volume, groupCount, apply) {
    if (Volume.isLociEmpty(loci) || !Volume.areEquivalent(loci.volume, volume))
        return false;
    ...
    if (Interval.is(loci.instances)) {
        ...
        OrderedSet.forEach(loci.instances, instanceIndex => {
            ...
            if (apply(Interval.ofBounds(offset, offset + groupCount)))
                changed = true;
        });
    }
    return changed;
}
export function applySliceGroupIntervals(indices, offset, apply) {
    ...
    for (let i = 1, il = indices.length; i < il; ++i) {
        ...
        if (apply(Interval.ofBounds(start, prev + 1)))
            changed = true;
        start = value;
        prev = value;
    }
...
//
export function SliceVisual(materialId, volume, key) {
    return VolumeVisual({
