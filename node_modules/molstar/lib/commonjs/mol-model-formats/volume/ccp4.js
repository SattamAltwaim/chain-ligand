...
exports.getCcp4Size = getCcp4Size;
...
const parser_1 = require("../../mol-io/reader/ccp4/parser.js");
const typed_array_1 = require("../../mol-io/common/typed-array.js");
...
function getCcp4Size(header) {
    if (header.xLength === 0.0 && header.yLength === 0.0 && header.zLength === 0.0) {
        return linear_algebra_1.Vec3.create(header.NX, header.NY, header.NZ);
    }
    else {
        return linear_algebra_1.Vec3.create(header.xLength, header.yLength, header.zLength);
    }
}
...
function volumeFromCcp4(source, params) {
    return mol_task_1.Task.create('Create Volume', async (ctx) => {
        ...
        const volgrid = {
            transform: { kind: 'spacegroup', cell, fractionalBox: geometry_1.Box3D.create(origin_frac, linear_algebra_1.Vec3.add((0, linear_algebra_1.Vec3)(), origin_frac, dimensions_frac)) },
            cells: data,
            stats: {
                min: (Number.isNaN(header.AMIN) || calcStats) ? (0, array_1.arrayMin)(values) : header.AMIN,
                max: (Number.isNaN(header.AMAX) || calcStats) ? (0, array_1.arrayMax)(values) : header.AMAX,
                mean: (Number.isNaN(header.AMEAN) || calcStats) ? (0, array_1.arrayMean)(values) : header.AMEAN,
                sigma: (Number.isNaN(header.ARMS) || header.ARMS === 0) ? (0, array_1.arrayRms)(values) : header.ARMS
            },
            periodicity: linear_algebra_1.Vec3.isInteger(dimensions_frac) ? 'xyz' : 'none',
        };
        return {
