...
// TODO: better API
class PluginAnimationManager extends component_1.StatefulPluginComponent {
    ...
    getParams() {
        if (!this._params) {
            this._params = {
                current: param_definition_1.ParamDefinition.Select(this._animations[0] && this._animations[0].name, this._animations.map(a => [a.name, a.display.name]), { label: 'Animation' })
            };
        }
        return this._params;
    }
    ...
    stopStateTransitionAnimation() {
        if (!this.isAnimatingStateTransition)
            return;
        return this.stop();
    }
    get isAnimating() {
