"use strict";
/**
 * Copyright (c) 2026 mol* contributors, licensed under MIT, See LICENSE file for more info.
 *
 * @author Alexander Rose <alexander.rose@weirdbyte.de>
 */
Object.defineProperty(exports, "__esModule", { value: true });
exports.setStructureWiggle = setStructureWiggle;
exports.clearStructureWiggle = clearStructureWiggle;
exports.setStructureWiggleFromUncertainty = setStructureWiggleFromUncertainty;
const structure_1 = require("../../mol-model/structure.js");
const transforms_1 = require("../transforms.js");
const mol_state_1 = require("../../mol-state/index.js");
const loci_1 = require("../../mol-model/loci.js");
const wiggle_1 = require("../../mol-theme/wiggle.js");
const int_1 = require("../../mol-data/int.js");
const WiggleManagerTag = 'wiggle-controls';
async function setStructureWiggle(plugin, components, value, lociGetter, types) {
    await eachRepr(plugin, components, async (update, repr, wiggleCell) => {
        if (types && types.length > 0 && !types.includes(repr.params.values.type.name))
            return;
        const structure = repr.obj.data.sourceData;
        // always use the root structure to get the loci so the wiggle
        // stays applicable as long as the root structure does not change
        const loci = await lociGetter(structure.root);
        if (loci_1.Loci.isEmpty(loci) || (0, loci_1.isEmptyLoci)(loci))
            return;
        const layer = {
            bundle: structure_1.StructureElement.Bundle.fromLoci(loci),
            value,
        };
        if (wiggleCell) {
            const bundleLayers = [...wiggleCell.params.values.layers, layer];
            const filtered = getFilteredBundle(bundleLayers, structure);
            update.to(wiggleCell).update(wiggle_1.Wiggle.toBundle(filtered));
        }
        else {
            const filtered = getFilteredBundle([layer], structure);
            update.to(repr.transform.ref)
                .apply(transforms_1.StateTransforms.Representation.WiggleStructureRepresentation3DFromBundle, wiggle_1.Wiggle.toBundle(filtered), { tags: WiggleManagerTag });
        }
    });
}
async function clearStructureWiggle(plugin, components, types) {
    await eachRepr(plugin, components, async (update, repr, wiggleCell) => {
        if (types && types.length > 0 && !types.includes(repr.params.values.type.name))
            return;
        if (wiggleCell) {
            update.delete(wiggleCell.transform.ref);
        }
    });
}
async function eachRepr(plugin, components, callback) {
    const state = plugin.state.data;
    const update = state.build();
    for (const c of components) {
        for (const r of c.representations) {
            const wiggle = state.select(mol_state_1.StateSelection.Generators.ofTransformer(transforms_1.StateTransforms.Representation.WiggleStructureRepresentation3DFromBundle, r.cell.transform.ref).withTag(WiggleManagerTag));
            await callback(update, r.cell, wiggle[0]);
        }
    }
    return update.commit({ doNotUpdateCurrent: true });
}
/** filter wiggle layers for given structure */
function getFilteredBundle(layers, structure) {
    const wiggle = wiggle_1.Wiggle.ofBundle(layers, structure.root);
    const merged = wiggle_1.Wiggle.merge(wiggle);
    return wiggle_1.Wiggle.filter(merged, structure);
}
function getUncertaintyValue(unit, element) {
    if (structure_1.Unit.isAtomic(unit)) {
        return unit.model.atomicConformation.B_iso_or_equiv.value(element);
    }
    else if (structure_1.Unit.isSpheres(unit)) {
        return unit.model.coarseConformation.spheres.rmsf[element];
    }
    return 0;
}
/** Compute min/max uncertainty (B-factor or RMSF) across all units in a structure */
function getUncertaintyRange(structure) {
    let min = Infinity;
    let max = -Infinity;
    for (const unit of structure.units) {
        const elements = unit.elements;
        for (let j = 0, jl = elements.length; j < jl; j++) {
            const v = getUncertaintyValue(unit, elements[j]);
            if (v < min)
                min = v;
            if (v > max)
                max = v;
        }
    }
    if (!isFinite(min))
        min = 0;
    if (!isFinite(max))
        max = 0;
    return { min, max };
}
/**
 * Set per-group wiggle based on B-factor/RMSF uncertainty data.
 * Values are normalized to [0, 1] within each structure's min-max range.
 * @param scale - maximum wiggle value (default 1.0, corresponds to Angstroms when combined with wiggleAmplitude)
 */
async function setStructureWiggleFromUncertainty(plugin, components, scale = 1, types) {
    await eachRepr(plugin, components, async (update, repr, wiggleCell) => {
        if (types && types.length > 0 && !types.includes(repr.params.values.type.name))
            return;
        const structure = repr.obj.data.sourceData;
        const root = structure.root;
        const { min, max } = getUncertaintyRange(root);
        const range = max - min;
        if (range <= 0)
            return;
        // Group elements by discretized uncertainty bucket (256 levels for Uint8 texture)
        const buckets = new Map();
        for (const unit of root.units) {
            const elements = unit.elements;
            const unitBuckets = new Map();
            for (let j = 0, jl = elements.length; j < jl; j++) {
                const v = getUncertaintyValue(unit, elements[j]);
                const normalized = (v - min) / range;
                const bucket = Math.min(255, Math.round(normalized * 255));
                if (!unitBuckets.has(bucket))
                    unitBuckets.set(bucket, []);
                unitBuckets.get(bucket).push(j);
            }
            for (const [bucket, indices] of unitBuckets) {
                if (!buckets.has(bucket))
                    buckets.set(bucket, []);
                buckets.get(bucket).push({ unit, indices });
            }
        }
        // Create one layer per bucket
        const bundleLayers = [];
        for (const [bucket, unitIndices] of buckets) {
            const value = (bucket / 255) * scale;
            const elements = [];
            for (const { unit, indices } of unitIndices) {
                elements.push({
                    unit,
                    indices: int_1.OrderedSet.ofSortedArray(new Int32Array(indices)),
                });
            }
            const loci = structure_1.StructureElement.Loci(root, elements);
            if (!structure_1.StructureElement.Loci.isEmpty(loci)) {
                bundleLayers.push({
                    bundle: structure_1.StructureElement.Bundle.fromLoci(loci),
                    value,
                });
            }
        }
        if (bundleLayers.length === 0)
            return;
        const filtered = getFilteredBundle(bundleLayers, structure);
        if (wiggleCell) {
            update.to(wiggleCell).update(wiggle_1.Wiggle.toBundle(filtered));
        }
        else {
            update.to(repr.transform.ref)
                .apply(transforms_1.StateTransforms.Representation.WiggleStructureRepresentation3DFromBundle, wiggle_1.Wiggle.toBundle(filtered), { tags: WiggleManagerTag });
        }
    });
}
