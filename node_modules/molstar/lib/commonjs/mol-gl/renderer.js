...
exports.getTransformedLightDirection = getTransformedLightDirection;
const util_1 = require("../mol-canvas3d/camera/util.js");
...
const frustum3d_1 = require("../mol-math/geometry/primitives/frustum3d.js");
...
exports.RendererParams = {
    ...
    enableAnimation: param_definition_1.ParamDefinition.Boolean(true, { description: 'Enable time-based animations.' }),
};
...
function getTransformedLightDirection(light, t) {
    const tld = new Array(light.count * 3);
    for (let i = 0, il = light.count; i < il; ++i) {
        linear_algebra_1.Vec3.fromArray(tmpDir, light.direction, i * 3);
        linear_algebra_1.Vec3.transformDirection(tmpDir, tmpDir, t);
        linear_algebra_1.Vec3.toArray(tmpDir, tld, i * 3);
    }
    return tld;
}
var Renderer;
(function (Renderer) {
    ...
    function create(ctx, props = {}) {
        ...
        const model = (0, linear_algebra_1.Mat4)();
        ...
        const invHeadRotation = (0, linear_algebra_1.Mat4)();
        const modelViewEye = (0, linear_algebra_1.Mat4)();
        const invModelViewEye = (0, linear_algebra_1.Mat4)();
        const cameraDir = (0, linear_algebra_1.Vec3)();
        const cameraPosition = (0, linear_algebra_1.Vec3)();
        const cameraTarget = (0, linear_algebra_1.Vec3)();
        ...
        const globalUniforms = {
            ...
            uHasHeadRotation: mol_util_1.ValueCell.create(false),
            uInvHeadRotation: mol_util_1.ValueCell.create(invHeadRotation),
            uHasEyeCamera: mol_util_1.ValueCell.create(false),
            uModelViewEye: mol_util_1.ValueCell.create(modelViewEye),
            uInvModelViewEye: mol_util_1.ValueCell.create(invModelViewEye),
            uIsAsymmetricProjection: mol_util_1.ValueCell.create(false),
            uIsOrtho: mol_util_1.ValueCell.create(1),
            uViewOffset: mol_util_1.ValueCell.create(viewOffset),
            uModelScale: mol_util_1.ValueCell.create(1),
            ...
            uTime: mol_util_1.ValueCell.create(0),
            uEnableAnimation: mol_util_1.ValueCell.create(p.enableAnimation),
        };
        ...
        const renderObject = (r, variant, flag) => {
            ...
            geometry_1.Sphere3D.scaleNX(boundingSphere, r.values.boundingSphere.ref.value, modelScale);
            if (!frustum3d_1.Frustum3D.intersectsSphere3D(frustum, boundingSphere)) {
                return;
            }
            ...
            if (minDistance !== 0 || maxDistance !== 0) {
                ...
                if (d + radius < minDistance * modelScale)
                    return;
                if (d - radius > maxDistance * modelScale)
                    return;
            }
        ...
        const update = (camera, scene) => {
            ...
            mol_util_1.ValueCell.updateIfChanged(globalUniforms.uModelScale, camera.scale);
            mol_util_1.ValueCell.update(globalUniforms.uCameraPosition, linear_algebra_1.Mat4.getTranslation(cameraPosition, invView));
            linear_algebra_1.Vec3.scale(cameraTarget, camera.state.target, camera.scale);
            linear_algebra_1.Vec3.normalize(cameraDir, linear_algebra_1.Vec3.sub(cameraDir, cameraTarget, cameraPosition));
            mol_util_1.ValueCell.update(globalUniforms.uCameraDir, cameraDir);
            ...
            if (hasHeadRotation) {
                mol_util_1.ValueCell.updateIfChanged(globalUniforms.uHasHeadRotation, true);
                mol_util_1.ValueCell.update(globalUniforms.uInvHeadRotation, linear_algebra_1.Mat4.invert(invHeadRotation, camera.headRotation));
                mol_util_1.ValueCell.update(globalUniforms.uLightDirection, getTransformedLightDirection(light, invHeadRotation));
            }
            else {
                mol_util_1.ValueCell.updateIfChanged(globalUniforms.uHasHeadRotation, false);
                mol_util_1.ValueCell.updateIfChanged(globalUniforms.uInvHeadRotation, linear_algebra_1.Mat4.id);
                mol_util_1.ValueCell.update(globalUniforms.uLightDirection, light.direction);
            }
            mol_util_1.ValueCell.update(globalUniforms.uIsAsymmetricProjection, camera.isAsymmetricProjection);
        };
        const updateInternal = (group, camera, depthTexture, renderMask, markingDepthTest) => {
            ...
            mol_util_1.ValueCell.update(globalUniforms.uModel, linear_algebra_1.Mat4.scaleUniformly(model, group.view, modelScale));
            mol_util_1.ValueCell.update(globalUniforms.uModelView, linear_algebra_1.Mat4.mul(modelView, camera.view, model));
            ...
            const hasEyeCamera = !linear_algebra_1.Mat4.isZero(camera.viewEye);
            if (hasEyeCamera) {
                mol_util_1.ValueCell.updateIfChanged(globalUniforms.uHasEyeCamera, true);
                mol_util_1.ValueCell.update(globalUniforms.uModelViewEye, linear_algebra_1.Mat4.mul(modelViewEye, camera.viewEye, model));
                mol_util_1.ValueCell.update(globalUniforms.uInvModelViewEye, linear_algebra_1.Mat4.invert(invModelViewEye, modelViewEye));
            }
            else {
                mol_util_1.ValueCell.updateIfChanged(globalUniforms.uHasEyeCamera, false);
                mol_util_1.ValueCell.updateIfChanged(globalUniforms.uModelViewEye, linear_algebra_1.Mat4.id);
                mol_util_1.ValueCell.updateIfChanged(globalUniforms.uInvModelViewEye, linear_algebra_1.Mat4.id);
            }
        ...
        return {
            ...
            setTime: (time) => {
                mol_util_1.ValueCell.updateIfChanged(globalUniforms.uTime, time);
            },
            setProps: (props) => {
                ...
                if (props.enableAnimation !== undefined && props.enableAnimation !== p.enableAnimation) {
                    ...
                    mol_util_1.ValueCell.update(globalUniforms.uEnableAnimation, p.enableAnimation);
                }
            },
